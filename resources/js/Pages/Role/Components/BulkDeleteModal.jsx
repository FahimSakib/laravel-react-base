...
export default function BulkDeleteModal({ setShowBulkDeleteModal, selectedItems, setSelectedItems }) {
    ...
    const deleteRoles = () => {
        router.post(route('roles.bulk.delete'), selectedItems, {
            preserveScroll: true,
            onStart: () => {
                setLoading(true)
            },
    ...
    const roleOrRoles = selectedItems.ids.length > 1 ? 'roles' : 'role'
    ...
    return (
        <BasicModal>
            <div className="">
                ...
                <InfoBanner
                    text={`All the related data for ${roleOrRoles === 'role' ? 'this' : ' these'} ${roleOrRoles} will also be deleted.`}
                    canHide={false}
                    className="mb-4"
                />
                <div>
                    <p className="text-xs pl-1 pb-1 text-[#6c737f] dark:text-[#a0aec0]">
                        To confirm, type "<strong>delete {selectedItems.ids.length} {roleOrRoles}</strong>" in the box below
                    </p>
                    <TextInputDefault
                        value={confirmValue}
                ...
                <div className="my-4 flex flex-col sm:flex-row gap-2">
                    <button
                        ...
                        disabled={(confirmValue !== `delete ${selectedItems.ids.length} ${roleOrRoles}`) || loading}
                        onClick={deleteRoles}
                    >
